/*
  ...
  This version has been corrected to work with arduino 1.0. The code has 
  not been tested and further developement shall be made for specific
  applications.
*/
...
SerialLCD::SerialLCD(uint8_t tx, uint8_t rx):SoftwareSerial(tx,rx)
{
...
void SerialLCD::begin()
{
	...
	while(1)
    {
        ...
        if (SoftwareSerial::available() > 0 &&SoftwareSerial::read()==byte(SLCD_INIT_DONE))
            break;
    }
    delay(2);
}
